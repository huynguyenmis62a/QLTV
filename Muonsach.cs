...
namespace QLTV
{
    public partial class Muonsach : Form
    {
        ...
        private void btnDeletem_Click_1(object sender, EventArgs e)
        {
        ...
        private void btnMuon_Click_1(object sender, EventArgs e)
        {
            try
            {
                sql = " insert into TableMuonTraSach (madocgia, NgayMuon, masach, NgayTra, masophieumuon)" +
                "Values('" + txtmadocgia.Text + "',N'" + txtNgaymuon.Value + "',N'" + txtmasach.Text + "',N'"
                 + txtHantra.Value + "',N'" + txtMaphieumuon.Text + "')";
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
                ...
                sql = "select TableDocGia.madocgia,TableDocGia.HoTen, TableMuontrasach.masach," +
                     " TableSach.TenSach,TableSach.TacGia,TableMuonTraSach.NgayMuon,TableSach.NXB," +
                     "TableSach.TheLoai,TableMuonTraSach.NgayTra from TableMuontrasach, TableDocGia, TableSach where TableMuontrasach.masach = TableSach.masach and TableMuontrasach.madocgia = TableDocGia.madocgia";
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
                da = new SqlDataAdapter(sql, conn);
        ...
        private void NapCT()
        {
            ...
            txtMaphieumuon.Text = grdData1.Rows[i].Cells["masophieumuon"].Value.ToString();
